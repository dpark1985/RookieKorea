...
.controller('sportsCtrl', ['$scope', '$window', '$location', '$http', function ($scope, $window, $location, $http){
	...
	$http.get('/model/'+curLocation+'/competitions').
	then(function(response) {
		...
		$http.get('/model/numbers/'+curLocation).
		then(function(response) {
		    // this callback will be called asynchronously
		    // when the response is available
	...
	if (curLocation === 'baseball'){ $scope.curLocation = {ko: '야구', en: 'baseball'}; } 
	else if (curLocation === 'basketball'){ $scope.curLocation = {ko: '농구', en: 'basketball'}; } 
	else if (curLocation === 'bedminton'){ $scope.curLocation = {ko: '배드민턴', en: 'bedminton'}; } 
	else if (curLocation === 'bowling'){ $scope.curLocation = {ko: '볼링', en: 'bowling'}; } 
	else if (curLocation === 'soccer'){ $scope.curLocation = {ko: '축구', en: 'soccer'}; } 
	else if(curLocation === 'tennis'){ $scope.curLocation = {ko: '테니스', en: 'tennis'}; } 
	else if(curLocation === 'market'){ $scope.curLocation = {ko: '마켓', en: 'market'}; }
	...
	*/
	...
	$scope.subTab2 = function(){
		...
		$http.get('/model/'+curLocation+'/courts').
		then(function(response) {
		    // this callback will be called asynchronously
		    // when the response is available
		    $scope.items = response.data;
		    console.log($scope.items);
		    for(var i in $scope.items){
		    	$scope.items[i].courttImg = $scope.items[i].courtImg.replace("public", "");
		    }
		}, function(response) {
		    // called asynchronously if an error occurs
		    // or server returns response with an error status.
	...
	$scope.subTab3 = function(){
		...
		$http.get('/model/'+curLocation+'/clubs').
		then(function(response) {
		    // this callback will be called asynchronously
		    // when the response is available
		    $scope.items = response.data;
	...
	$scope.stateChange = function(){
		...
		if($scope.state.seoul){
			$scope.selectedState.push('서울특별시');
		}
		if ($scope.state.busan){
			$scope.selectedState.push('부산광역시');
		} 
		if ($scope.state.daegue){
			$scope.selectedState.push('대구광역시');
		} 
		if ($scope.state.incheon){
			$scope.selectedState.push('인천광역시');
		} 
		if ($scope.state.gwangju){
			$scope.selectedState.push('광주광역시');
		}
		if ($scope.state.daejeon){
			$scope.selectedState.push('대전광역시');
		} 
		if ($scope.state.ulsan){
			$scope.selectedState.push('울산광역시');
		} 
		if ($scope.state.jeju){
			$scope.selectedState.push('제주도');
		} 
		if ($scope.state.gyeonggi){
			$scope.selectedState.push('경기도');
		} 
		if ($scope.state.ganwon){
			$scope.selectedState.push('강원도');
		} 
		if ($scope.state.chungbuk){
			$scope.selectedState.push('충청북도');
		} 
		if ($scope.state.chungnam){
			$scope.selectedState.push('충청남도');
		} 
		if ($scope.state.jeonbuk){
			$scope.selectedState.push('전라북도');
		} 
		if ($scope.state.jeonnam){
			$scope.selectedState.push('전라남도');
		} 
		if ($scope.state.gyeongbuk){
			$scope.selectedState.push('경상북도');
		} 
		if ($scope.state.gyeongnam){
			$scope.selectedState.push('경상남도');
		}
